...
export async function fermerMonTicketController(
    req: Request<{}, baseResponse, fermerTicketUtilisateurBody>,
    res: Response<baseResponse>
) {
    try {
        if (!req.user) {
        ...
        const resultat = await fermerTicketUtilisateur(ticketId, req.user.id);
